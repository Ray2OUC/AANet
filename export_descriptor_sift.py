...
def export(images_list, keypoints_type, num_keypoints, detection_thresh, extension,
           resize=False, h=480, w=640):

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    # Load my descriptor model
    checkpoint_path = '/home/ouc/aa.pth'
    descriptor = AANet()
    ...
    for img_path in tqdm(image_files):
        ...
        #inputs = img
        with torch.no_grad():
            outputs = descriptor.forward(img)
            descs = {}
            descs['descriptors'] = outputs
            descriptors=[]
            for k in descs.keys():
                desc = func.normalize(func.grid_sample(descs[k], grid_points),dim=1).squeeze().cpu().numpy().transpose(1, 0)
                #desc = func.normalize(func.grid_sample(descs[k], grid_points), dim=1)[0, :, :, 0].t().cpu().numpy()
...
if __name__ == "__main__":
    ...
    export(args.images_list, keypoints_type, num_keypoints, args.detection_thresh, extension,
